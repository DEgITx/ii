...
//
// Цель: пропустить изображение через модель (например INT8-квантованную
// yolov8m_int8.tflite) и получить «сырые» выходы.
//
...
// инференса. Без этого сигнал может прилететь посреди poll() в драйвере
// внешнего делегата, тот возвращает EINTR и репортит «Inference poll
// timeout» — ускоритель остаётся в неконсистентном состоянии и часто
// требует переинициализации. С блокировкой SIGINT он просто ставится
