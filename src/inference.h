...
// Фабрика по имени. backend = "" возвращает «дефолтный» бэкенд по
// статическому приоритету (см. .cpp), НЕ проверяя его работоспособность —
// для авто-выбора с фактической проверкой используйте load_best_engine().
...
// Авто-выбор «наиболее удачного работоспособного» бэкенда: перебирает
// собранные бэкенды по приоритету (ускоренные GPU → CPU:
// tensorrt → directml → tflite → ii) и возвращает ПЕРВЫЙ, который реально
// загрузил модель этими opts. Так учитываются и формат модели, и наличие
// железа/делегата: если load() падает, идём к следующему кандидату.
// Возвращаемый Engine уже загружен (load() повторно вызывать не нужно).
// Если задан chosen_backend, в него пишется имя выбранного бэкенда.
// nullptr — ни один собранный бэкенд не смог загрузить модель.
std::unique_ptr<Engine> load_best_engine(const std::string& model_path,
                                         const Engine::Options& opts,
                                         std::string* chosen_backend = nullptr);

// Список бэкендов, собранных в текущий бинарь. Удобно для --help / UI.
std::vector<std::string> available_backends();

// Приоритетный порядок авто-выбора (best → worst), пересечённый с
// собранными бэкендами. Тот же список, что перебирает load_best_engine.
